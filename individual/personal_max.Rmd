...
# Filter for 1996
details_1996 <- details_selected %>% filter(year == 1996)
details_grouped_1996 <- details_1996 %>%
...
global_averages_1996 <- details_grouped_1996 %>%
  summarise(
...
details_grouped_1996 <- details_grouped_1996 %>%
  mutate(
    fatality_group = if_else(avg_fatalities_by_type > global_averages_1996$global_avg_fatalities,
                             "More Deadly", "Less Deadly"),
    damage_group = if_else(avg_damage_by_type > global_averages_1996$global_avg_damage,
                           "More Damaging", "Less Damaging")
  )

# Adjusted Graph for 1996 Fatalities
fatality_graph_1996 <- details_grouped_1996 %>%
  ggplot(aes(x = fatality_group, 
             y = avg_fatalities_by_type, 
             fill = fatality_group)) +
  stat_summary(fun = "mean", geom = "bar", position = "dodge", alpha = 0.7) +
  labs(title = "1996: Fatalities by Group", subtitle = "Data from 1996", 
       x = "Weather Group",  # Renamed the x-axis
...
damage_graph_1996 <- details_grouped_1996 %>%
  ggplot(aes(x = damage_group, 
             y = avg_damage_by_type, 
             fill = damage_group)) +
  stat_summary(fun = "mean", geom = "bar", position = "dodge", alpha = 0.7) +
  labs(title = "1996: Damages by Group", subtitle = "Data from 1996", 
       x = "Weather Group",  # Renamed the x-axis
...
# Filter for 2014
details_2014 <- details_selected %>% filter(year == 2014)
details_grouped_2014 <- details_2014 %>%
  mutate(fatalities_total = deaths_direct + deaths_indirect) %>%
  group_by(event_type) %>%
  summarise(
    avg_fatalities_by_type = mean(fatalities_total, na.rm = TRUE),
...
global_averages_2014 <- details_grouped_2014 %>%
  summarise(
    global_avg_fatalities = mean(avg_fatalities_by_type, na.rm = TRUE),
    global_avg_damage = mean(avg_damage_by_type, na.rm = TRUE)
  )
...
details_grouped_2014 <- details_grouped_2014 %>%
  mutate(
    fatality_group = if_else(avg_fatalities_by_type > global_averages_2014$global_avg_fatalities,
                             "More Deadly", "Less Deadly"),
    damage_group = if_else(avg_damage_by_type > global_averages_2014$global_avg_damage,
                           "More Damaging", "Less Damaging")
  )

# Adjusted Graph for 2014 Fatalities
fatality_graph_2014 <- details_grouped_2014 %>%
  ggplot(aes(x = fatality_group, 
             y = avg_fatalities_by_type, 
             fill = fatality_group)) +
  stat_summary(fun = "mean", geom = "bar", position = "dodge", alpha = 0.7) +
  labs(title = "2014: Fatalities by Group", subtitle = "Data from 2014", 
       x = "Weather Group",  # Renamed the x-axis
       y = "Average Fatalities") +
  theme_minimal() +
  theme(
...
damage_graph_2014 <- details_grouped_2014 %>%
  ggplot(aes(x = damage_group, 
             y = avg_damage_by_type, 
             fill = damage_group)) +
  stat_summary(fun = "mean", geom = "bar", position = "dodge", alpha = 0.7) +
  labs(title = "2014: Damages by Group", subtitle = "Data from 2014", 
       x = "Weather Group",  # Renamed the x-axis
       y = "Average Damages (USD)") +
  theme_minimal() +
  theme(
...
combined_1996 <- fatality_graph_1996 + damage_graph_1996
combined_2014 <- fatality_graph_2014 + damage_graph_2014
